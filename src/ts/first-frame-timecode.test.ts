...
// setFirstFrameTimecode stamps the first buffer and can read its clock-bridge
// base_time from a caller-supplied element instead of the stamper. This lets a
// caller whose frame running-time is relative to a DIFFERENT pipeline keep
// `runningTimeNs + baseTimeNs` in one clock domain.
...
describe.skipIf(!hasTimecodeStamper)("setFirstFrameTimecode baseTimeElement", () => {
  ...
  it("reports baseClockMatched=true when the base element shares the stamper's clock (separate pipelines on the system clock)", async () => {
    // When two independent pipelines both run on the global GstSystemClock, a base
    // element from one still shares the stamper's clock, so the base-time
    // subtraction is valid and baseClockMatched is true.
    const rec = new Pipeline(
      "videotestsrc num-buffers=10 is-live=true ! timecodestamper name=tc_stamper ! fakesink",
    );
    const ext = new Pipeline("videotestsrc is-live=true ! fakesink name=fsExt");
